...
public class SqliRoleValidatorInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		Utilisateur utilisateur = (Utilisateur) invocation.getInvocationContext().getSession().get(SqliBasicAction.SESSION_USER_FIELD);
		...
		if(isUtilisateurInRole(utilisateur)){
			if(isUtilisateurHasActions(utilisateur)){
				...
				((SqliBasicAction) invocation.getAction()).addActionError(((SqliBasicAction) invocation.getAction()).getText("role.error.unauthorized"));
				return SqliBasicAction.PARTIAL_AUTHORIZATION_REQUIRED_RESULT;
			}
		}
		
		return SqliBasicAction.COMPLETE_AUTHORIZATION_REQUIRED_RESULT;
	}
